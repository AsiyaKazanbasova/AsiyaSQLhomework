...
кот “Мурзик” (попробуйте выполнить 2 способами)
*/
SELECT s.`shopname` AS shop_name
FROM `shops` s
JOIN `cats` c ON s.id = c.shops_id
WHERE c.`name` = "Murzik";
-- 2й способ
SELECT s.`shopname` AS shop_name
FROM `shops` s
JOIN (SELECT `name`, `id`, shops_id FROM `cats`
WHERE `name` = "Murzik") c 
ON s.id = c.shops_id;
/*
3. Вывести магазины, в которых 
НЕ продаются коты “Мурзик” и “Zuza”
*/
SELECT s.`shopname`
FROM `shops` s
LEFT JOIN `cats` c
